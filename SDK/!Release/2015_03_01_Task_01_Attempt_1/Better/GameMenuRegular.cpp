...
//Create Demo smt
    ...
    for(int i=0;i<4;i++){
        Data->vertex[3*i]=tempVert[8*i];
        Data->vertex[3*i+1]=tempVert[8*i+1];
        Data->vertex[3*i+2]=tempVert[8*i+2];
        Data->textures[2*i]=tempVert[8*i+3];
        Data->textures[2*i+1]=tempVert[8*i+4];}
    for(int i=0;i<6;i++)
...
//Add element
    ...
    for(int i=0;i<4;i++){
        Elem[Elemi]->Data[0].vertex[3*i]=tempVert2[8*i];
        Elem[Elemi]->Data[0].vertex[3*i+1]=tempVert2[8*i+1];
        Elem[Elemi]->Data[0].vertex[3*i+2]=tempVert2[8*i+2];
        Elem[Elemi]->Data[0].textures[2*i]=tempVert2[8*i+3];
        Elem[Elemi]->Data[0].textures[2*i+1]=tempVert2[8*i+4];}
    for(int i=0;i<6;i++)
    ...
    for(int i=0;i<4;i++){
        Elem[Elemi]->Data[1].vertex[3*i]=tempVert2[8*i];
        Elem[Elemi]->Data[1].vertex[3*i+1]=tempVert2[8*i+1];
        Elem[Elemi]->Data[1].vertex[3*i+2]=tempVert2[8*i+2];
        Elem[Elemi]->Data[1].textures[2*i]=tempVert2[8*i+3];
        Elem[Elemi]->Data[1].textures[2*i+1]=tempVert2[8*i+4];}
    for(int i=0;i<6;i++)
...
//Another
    ...
    tempVert[27]*=2;
    for(int i=0;i<4;i++){
        Elem[Elemi]->Data[0].vertex[3*i]=tempVert3[8*i];
        Elem[Elemi]->Data[0].vertex[3*i+1]=tempVert3[8*i+1];
        Elem[Elemi]->Data[0].vertex[3*i+2]=tempVert3[8*i+2];
        Elem[Elemi]->Data[0].textures[2*i]=tempVert3[8*i+3];
        Elem[Elemi]->Data[0].textures[2*i+1]=tempVert3[8*i+4];}
    for(int i=0;i<6;i++)
        Elem[Elemi]->Data[0].face[i]=tempFace[i];
    for(int i=0;i<32;i+=8){
    ...
    for(int i=0;i<4;i++){
        Elem[Elemi]->Data[1].vertex[3*i]=tempVert3[8*i];
        Elem[Elemi]->Data[1].vertex[3*i+1]=tempVert3[8*i+1];
        Elem[Elemi]->Data[1].vertex[3*i+2]=tempVert3[8*i+2];
        Elem[Elemi]->Data[1].textures[2*i]=tempVert3[8*i+3];
        Elem[Elemi]->Data[1].textures[2*i+1]=tempVert3[8*i+4];}
    for(int i=0;i<6;i++)
        Elem[Elemi]->Data[1].face[i]=tempFace[i];
    Elem[Elemi]->Data[0].facei=6;
    Elem[Elemi]->Data[1].facei=6;
//
    ...
    for(int i=0;i<4;i++){
        temp3->Data->vertex[3*i]=tempVert[8*i];
        temp3->Data->vertex[3*i+1]=tempVert[8*i+1];
        temp3->Data->vertex[3*i+2]=tempVert[8*i+2];
        temp3->Data->textures[2*i]=tempVert[8*i+3];
        temp3->Data->textures[2*i+1]=tempVert[8*i+4];}
    for(int i=0;i<6;i++)
...
void GameMenuRegular::Draw()
{
    ...
    pGL->ClearGLScene();
    pGL->EnableMode(1);
    pGL->DrawGLScene(Data->vertex,Data->textures,0,Data->face,Data->facei,background,0);
    for(int i=0;i<Elemi;i++){
        temp=(i==ActiveItem)?1:0;
        pGL->DrawGLScene(Elem[i]->Data[temp].vertex,Elem[i]->Data[temp].textures,0,Elem[i]->Data[temp].face,Elem[i]->Data[temp].facei,-1,2);}
    pGL->ShowGLScene();
}
